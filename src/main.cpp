...
//DE ACA PARA ABAJO ESTA LO QUE TIENE QUE VER CON TELEGRAM
  CTBot miBot;
  ...
  void envioTelegram();//declaracion de la funcion que se encarga de comprobar si la temperatura esta dentro de los rangos deseados y dar aviso por telegram

  unsigned long previousMillisTelegram = 0;
  
  unsigned int intervalTelegram = 120000;
...
//ACA ESTA LO RELACIONADO CON LA LECTURA DE TEMPERATURA
...
// DECLARACION DE VARIABLES PARA PROGRAMA
  int histeresis = 1;
  int set = 19;
  bool fueraDeRango;
...
//ACA COMIENZA LO QUE TIENE QUE VER CON FIREBASE
  ...
  botonesTelegram.addButton("Estoy en conocimiento", "administrarNotificaciones", CTBotKeyboardButtonQuery);
...
//intervalLeer=3seg
  ...
  //intervalPublicar=10min
    ...
    switch (tanqueNumero){
      case 1:
        valores[0] =850;// analogRead(adcPins[tanqueNumero]);
        tempTanque[0] = convertir_rango(valores[0]);
        tanqueNumero = 2;
      break;
      case 2:
        valores[1] = 845;//analogRead(adcPins[tanqueNumero]);
        tempTanque[1] = convertir_rango(valores[1]);
        tanqueNumero = 3;
        break;
      case 3:
        valores[2] = 830;//analogRead(adcPins[tanqueNumero]);
        tempTanque[2] = convertir_rango(valores[2]);
        tanqueNumero = 4;
        break;
      case 4:
        valores[3] = 1000;//analogRead(adcPins[tanqueNumero]);
        tempTanque[3] = convertir_rango(valores[3]);
        tanqueNumero = 5;
        break;
      case 5:
        valores[4] = 963;//analogRead(adcPins[tanqueNumero]);
        tempTanque[4] = convertir_rango(valores[4]);
        tanqueNumero = 6;
        break;
      case 6:
        valores[5] = 526;//analogRead(adcPins[tanqueNumero]);
        tempTanque[5] = convertir_rango(valores[5]);
        tanqueNumero = 1;
        break;
    }
  }

  if (millis() - previousMillisTelegram >= intervalTelegram){

    for (int i = 0; i < 6; ++i) {
      if (tempTanque[i] < bandaBaja || tempTanque[i] > bandaAlta){
        fueraDeRango = true;
      }
    }
...
void envioTelegram(){
  sprintf(mensaje, "Hay temperaturas fuera de rango, revisar cuanto antes");
  miBot.sendMessage(ID_CHAT, mensaje, botonesTelegram); //Manda este mensaje a telegram, al id correspondiente  
}